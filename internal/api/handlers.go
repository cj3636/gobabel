...
func (h Handler) Alphabet(w http.ResponseWriter, r *http.Request) {
	...
	jsonw(w, map[string]any{
		"id":            a.ID,
		"page_size":     engine.PageSize,
		"length":        len(a.Chars),
		"chars_escaped": a.EscapedChars(),
	})
}
...
func (h Handler) Validate(w http.ResponseWriter, r *http.Request) {
	...
	jsonw(w, map[string]any{
		"valid":      len(inv) == 0 && len(req.Text) <= engine.PageSize,
		"length":     len(req.Text),
		"max_length": engine.PageSize,
		"alphabet":   h.Engine.Alphabet.ID,
		"invalid":    inv,
	})
}
...
func (h Handler) Book(w http.ResponseWriter, r *http.Request) {
	...
	if rg != nil {
		jsonw(w, map[string]any{
			"address_type": engine.AddressType,
			"start":        rg[0],
			"end":          rg[1],
			"text":         string(page[rg[0]:rg[1]]),
		})
		return
	}
	...
	jsonw(w, map[string]any{
		...
		"text":         string(page),
		"page_size":    engine.PageSize,
		"alphabet":     alphabet.CodeASCIIV1ID,
		"codec":        codec.ID,
	})
}
...
func (h Handler) Corpus(w http.ResponseWriter, r *http.Request) {
	...
	if p[0] == "" || p[2] == "" || p[4] == "" || p[6] == "" || p[7] == "" ||
		strings.Contains(p[7], ":") ||
		p[1] != "wall" || p[3] != "shelf" || p[5] != "book" {
		writeErr(w, 404, engine.ErrInvalidAddress.Error(), "Invalid corpus path.", nil)
		return
	}
	...
	if len(p) == 9 {
		...
		jsonw(w, map[string]any{
			"address_type": corpus.AddressType,
			"start":        s,
			"end":          e,
			"text":         string(page[s:e]),
		})
		return
	}
